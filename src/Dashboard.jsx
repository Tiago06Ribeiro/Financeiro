...
export default function Dashboard({ userEmail, onLogout }) {
  ...
  function usePersisted(key, defaultValue) {
    const stored = localStorage.getItem("fin_" + key);
    ...
    function set(valOrFn) {
      setVal(prev => {
        ...
        localStorage.setItem("fin_" + key, JSON.stringify(next));
        return next;
      });
    }
    return [val, set];
  ...
  const [catsGasto, setCatsGasto] = usePersisted("catsGasto", DEFAULT_CATS_GASTO);
  const [contas, setContas] = usePersisted("contas", DEFAULT_CONTAS);
  const [orcamentos, setOrcamentos] = usePersisted("orcamentos", DEFAULT_ORCAMENTOS);
  const [receitas, setReceitas] = usePersisted("receitas", DEFAULT_RECEITAS);
  const [realizados, setRealizados] = useState(DEFAULT_REALIZADOS);
  const [gastos, setGastos] = usePersisted("gastos", []);
  const [gastosFixos, setGastosFixos] = usePersisted("gastosFixos", DEFAULT_GASTOS_FIXOS);
  const [pagosFixos, setPagosFixos] = usePersisted("pagosFixos", {});
  const [faturasPagas, setFaturasPagas] = usePersisted("faturasPagas", {}); // key: "contaId-ano-mes" -> {paga, dataPagamento} // key: "ano-mes-gfId" -> { pago, data }
